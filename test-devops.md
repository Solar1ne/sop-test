...
LLM можно. Каждая команда должна быть тебе понятна. На интервью спросим «почему `restart: unless-stopped` а не `always`», «что делает `set -euo pipefail`», «зачем `service_healthy` в depends_on». Не знаешь, говори честно, это нормально. 
