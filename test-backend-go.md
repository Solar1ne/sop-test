...
Ориентировочное время: 10-12 часов. Это не дедлайн, а честная прикидка. Если ушло больше, потому что хотелось разобраться глубже, просто напиши в сопроводительном письме. Мы ищем человека для прода, не марафонца.

## На чём писать

В проде у нас Go. Если ты Python-разработчик и хочешь попробовать, ок, пиши на FastAPI или Flask. Требования к архитектуре, безопасности и качеству остаются теми же, просто мысленно заменяй `pgx/v5` на `asyncpg`, `net/http` на `FastAPI`, `gofmt` на `ruff`. Смысл не меняется.

Учти, что при найме первые пару месяцев уйдут на погружение в Go. Язык сам по себе несложный, но писать идиоматично сразу не получится. Из-за этого Go-решение нам приятнее: видно, как ты мыслишь в стиле, в котором будешь работать.
...
Всё в одном `main.go` тоже заработает, но нам интересно посмотреть, как ты делишь ответственность. Обратная крайность тоже плохая: не городи 15 слоёв абстракций для обычного CRUD.
...
2. Что нужно на машине (Go 1.23+, Docker).
...
В сообщении: что получилось хорошо, что вызвало трудности, сколько реально ушло, что бы сделал иначе.

Обычно отвечаем за 3-5 рабочих дней. Если прошло больше недели, пни в личку, возможно сообщение потерялось.
